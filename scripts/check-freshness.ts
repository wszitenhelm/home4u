...
type FreshnessCheckResult = "LIVE" | "EXPIRED";
...
async function checkListingFreshness(sourceUrl: string): Promise<FreshnessCheckResult> {
  ...
  for (let offset = 0; offset < listings.length; offset += BATCH_SIZE) {
    ...
    await Promise.all(
      batch.map(async (listing) => {
        try {
          const result = await checkListingFreshness(listing.sourceUrl);
